...
class SchedulerTest {
    ...
    fun `los repasos vencidos entran en la cola`() {
        val cards = mapOf(
            80 to Card(rank = 80, box = 2, dueTurn = turno - 1000, seen = 3),
            90 to Card(rank = 90, box = 3, dueTurn = turno - 5000, seen = 4)
        )
        val cola = scheduler.buildQueue(catalog, cards, turno, size = 5).map { it.rank }
    ...
    fun `entre vencidas manda la caja mas baja`() {
        val cards = (1..20).associateWith { Card(it, box = 4, dueTurn = turno - 100, seen = 8) }
            .toMutableMap()
        cards[20] = Card(20, box = 1, dueTurn = turno - 10, seen = 2)

        val vencidasEnCola = scheduler.buildQueue(catalog, cards, turno, size = 6)
            .map { it.rank }
            .filter { it in cards }
    ...
    fun `la cola mezcla novedades con repasos en vez de agruparlas`() {
        ...
        val cards = (1..30).associateWith { Card(it, box = 2, dueTurn = turno - 100, seen = 2) }
        val cola = scheduler.buildQueue(catalog, cards, turno, size = 20)
    ...
    fun `las palabras nuevas entran por orden de frecuencia`() {
        val cola = scheduler.buildQueue(catalog, emptyMap(), turno, size = 4)
    ...
    fun `las novedades no salen agrupadas por nivel`() {
        ...
        val cola = scheduler.buildQueue(variado, emptyMap(), turno, size = 30)
    ...
    fun `reparte las novedades entre niveles en vez de agotar el primero`() {
        ...
        val niveles = scheduler.buildQueue(variado, emptyMap(), turno, size = 30)
            .map { it.cefr }.toSet()
    ...
    fun `siempre devuelve el tamano pedido mientras haya diccionario`() {
        val cola = scheduler.buildQueue(catalog, emptyMap(), turno, size = 30)
    ...
    fun `con todo dominado y sin vencer sigue habiendo preguntas`() {
        ...
        val cards = catalog.associate {
            it.rank to Card(it.rank, box = leitner.boxCount, dueTurn = turno + 1_000_000, seen = 9)
        }
        val cola = scheduler.buildQueue(catalog, cards, turno, size = 8)
    ...
    fun `lo no dominado se adelanta antes que lo dominado`() {
        val cards = mapOf(
            5 to Card(5, box = leitner.boxCount, dueTurn = turno + 1000, seen = 9),
            6 to Card(6, box = 2, dueTurn = turno + 5000, seen = 2)
        )
        val soloVistas = catalog.filter { it.rank in setOf(5, 6) }
        val cola = scheduler.buildQueue(soloVistas, cards, turno, size = 2)
        assertEquals(6, cola.first().rank)
    }

    @Test
    fun `una palabra no sale dos veces en la misma cola`() {
        val cards = mapOf(1 to Card(1, box = 1, dueTurn = turno - 1, seen = 1))
        val cola = scheduler.buildQueue(catalog, cards, turno, size = 6)
        assertEquals(cola.size, cola.map { it.rank }.toSet().size)
    }

    @Test
    fun `ignora progreso de palabras que ya no estan en el catalogo`() {
        val cards = mapOf(9999 to Card(9999, box = 1, dueTurn = turno - 1, seen = 1))
        val cola = scheduler.buildQueue(catalog, cards, turno, size = 3)
    ...
    fun `sin diccionario no hay cola`() {
        assertTrue(scheduler.buildQueue(emptyList(), emptyMap(), turno, size = 5).isEmpty())
    }

    @Test
    fun `dueCount no cuenta las que aun no se han visto`() {
        val cards = mapOf(
            1 to Card(1, dueTurn = turno - 1, seen = 0),
            2 to Card(2, dueTurn = turno - 1, seen = 3),
            3 to Card(3, dueTurn = turno + 1, seen = 3)
        )
        assertEquals(1, scheduler.dueCount(cards, turno))
    }
}
